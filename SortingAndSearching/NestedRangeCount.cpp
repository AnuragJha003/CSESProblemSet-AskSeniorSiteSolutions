...
int32_t main(){
    ...
    while(t--){
        ...
        }); //points in ascending order and if equal then y in descending order
//POLICY BASED DS   
        ...
        for(int i=0,j=n-1;i<n;i++,j--){
            aa[a[j][2]]=c.order_of_key({a[j][1]+1,-1}) ;//order_of_key number of elmnts less than a[j][1]+1
            c.insert({a[j][1],i});//insert into c 
        }
        
        for(int i=0;i<n;i++){//for the 2nd array thing 
